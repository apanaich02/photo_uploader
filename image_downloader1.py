...
def download_images():
    ...
    for index, file in enumerate(file_list, start=1):
        file_extension = os.path.splitext(file['title'])[1]
        new_filename = f"{selected_pharmacy}_{current_date}_ECO_{index}{file_extension}"
        file_path = os.path.join(local_folder, new_filename)
